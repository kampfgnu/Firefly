...
{
  return [self objectWithObjectID:objectID inContext:[NSManagedObjectContext contextForCurrentThread]];
}
...
{
	return [self findAllWithPredicate:searchTerm inContext:context sortDescriptors:nil];
}

+ (NSArray *)findAllWithPredicate:(NSPredicate *)searchTerm inContext:(NSManagedObjectContext *)context sortDescriptors:(NSArray *)sortDescriptors {
    NSFetchRequest *request = [self createFetchRequestInContext:context];
	[request setPredicate:searchTerm];
...
{
	return [self findAllWithPredicate:searchTerm sortDescriptors:nil];
}

+ (NSArray *)findAllWithPredicate:(NSPredicate *)searchTerm sortDescriptors:(NSArray *)sortDescriptors {
...
+ (NSArray *)findAllWithPredicateNotFaulted:(NSPredicate *)searchTerm {
  NSFetchRequest *request = [self createFetchRequestInContext:[NSManagedObjectContext contextForCurrentThread]];
  [request setPredicate:searchTerm];
...
+ (id)findFirstByUID:(id)searchValue inContext:(NSManagedObjectContext *)context {
  ...
  if ([searchValue isKindOfClass:[NSNumber class]]) {
    ...
    NSLog(@"Wrong type for searchValue!");
    return nil;
  }
...
+ (id)existingOrNewObjectWithAttribute:(NSString *)attribute matchingValue:(id)value {
    ...
    if (value != nil) {
        object = [[self class] findFirstByAttribute:attribute withValue:value];
    }
